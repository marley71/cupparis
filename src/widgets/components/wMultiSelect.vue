...
export default {
    ...
    methods: {
        getMultiSelectLabel(option) {
            ...
            return this.iconf.domainValues[option];
        },
        removeMultiSelect(option) {
            var id = this.iconf.value.indexOf(option);
            this.iconf.value.splice(id,1);
        },
    }
}
</script>
...
<template>
    <MultiSelect class="w-full" v-model="iconf.value" :options="iconf.options" optionLabel="name" optionValue="code"
                 :placeholder="iconf.placeholder" :filter="iconf.filter" display="chip" @change="onChange">

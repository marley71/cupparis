...
export default {
    ...
    waitStart(msg) {
        ...
        this.event().emit('wait-start',{msg:msg})
    },
    waitEnd() {
        console.log('CrudCore.waitEnd');
        this.event().emit('wait-end')
    },
    translate(key,plural,params) {
    ...
    sentenceCase(str) {
        ...
        return String(str)
            ...
            .replace(this.CAMEL_CASE_REGEXP, '$1 $2')
            // Add a space after any digits.
            .replace(this.TRAILING_DIGIT_REGEXP, '$1 $2')
            // Remove all non-word characters and replace with a single space.
            .replace(this.NON_WORD_REGEXP, ' ')
    ...
    alertError(msg,cTime) {
        ...
        console.error(msg);
        if (cTime || cTime !== 0) {
            let life = cTime?cTime:this.defaultAlertTime;
    ...
    messageDialog(msg,props,callbacks) {
        __dialog(this.useItems,'message',msg,props,callbacks);
    },

    errorDialog(msg,props,callbacks) {
        __dialog(this.useItems,'error',msg,props,callbacks);
    },

    warningDialog(msg,props,callbacks) {
        __dialog(this.useItems,'warning',msg,props,callbacks);
    },
    ...
    customDialog(msg,props,callbacks) {
        __dialog(this.useItems,'custom',msg,props,callbacks)
    },
...
function __dialog(useItems,type,msg,props,callbacks) {
    const div = document.createElement('div');
    document.body.appendChild(div);
    callbacks = callbacks || {};
    let comp = null;
